...
Парсер может требовать токены/ключи:
- `API_URL` (абсолютный URL до KinoServer, например `http://localhost:5000`)

Также парсер требует заранее скачанные данные фильмов. Каждый фильм разбит на 3 файла:
- `_film_data\kp_films\entity_*.json` - информация о фильме
- `_film_data\kp_staff\staff_*.json` - информация о персонале
- `_film_data\kp_reviews\reviews_*.json` - отзывы о фильме
- Папка `_film_data` должна находится в корне проекта.
